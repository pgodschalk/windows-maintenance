@{
  ...
  PrivateData       = @{
    PSData = @{
      ...
      ReleaseNotes = @'
1.1.0: Removed the long-running/heavy maintenance checks (storage SMART health,
free space, Disk Cleanup, system integrity SFC/DISM, and the full Defender scan)
along with the -FreeSpaceMinPercent parameter; the remaining alert-only checks
are critical event-log review, startup/scheduled-task drift, crash dumps, and
clock sync.
